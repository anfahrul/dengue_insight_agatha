...
<div class="container-fluid">
    ...
    $kordinatc1 = array_combine($lat_c2, $long_c2);
    $kordinatc2 = array_combine($lat_c3, $long_c3);
    $kordinatc3 = array_combine($lat_c1, $long_c1);
    ...
    <script>
        ...
        var redIcon = L.icon({
            iconUrl: '/map.png',
            iconSize: [25, 41], // ukuran ikon dalam piksel
            iconAnchor: [12, 41], // titik ancor ikon relatif terhadap posisi titik lokasi
        });
        const layerControl = L.control.layers(baseLayers, overlays).addTo(map);

        const parks = L.layerGroup();
        <?php foreach ($kordinatc2 as $key => $value) : ?>
            L.marker([<?= $key ?>, <?= $value ?>]).bindPopup('Ini Lokasi anak gizi lebih.').addTo(parks);
        <?php endforeach ?>
